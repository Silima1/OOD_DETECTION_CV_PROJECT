#### Script principal para avaliar MSP
import os
import torch
import pandas as pd
...
def plot_score_histogram(id_scores, ood_scores, ood_name):
    ...
    plt.ylabel("Density")
    plt.title(f"MSP Score Distribution: CIFAR-10 vs {ood_name.upper()}")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    save_path = f"results/score_distributions/msp_cifar10_vs_{ood_name}.png"
...
def plot_roc(id_scores, ood_scores, ood_name):
    ...
    plt.title(f"ROC Curve: CIFAR-10 vs {ood_name.upper()} using MSP")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    save_path = f"results/roc_curves/roc_msp_cifar10_vs_{ood_name}.png"
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def main():
    ...
    id_scores, id_labels, id_preds = compute_msp_scores(
        model=model,
    ...
    for ood_name in ood_names:
        ...
        ood_scores, ood_labels, ood_preds = compute_msp_scores(
            model=model,
    ...
    save_path = "results/tables/msp_ood_results.csv"
    df.to_csv(save_path, index=False)

    print("=" * 70)
    print("Resultados MSP:")
    print(df)
