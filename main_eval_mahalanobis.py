##########=================================================================================
'''
Criar script de avaliação Mahalanobis
'''
#########==================================================================================
import os
import torch
...
def plot_score_histogram(id_scores, ood_scores, ood_name):
    ...
    plt.ylabel("Density")
    plt.title(f"Mahalanobis Score Distribution: CIFAR-10 vs {ood_name.upper()}")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    save_path = f"results/score_distributions/mahalanobis_cifar10_vs_{ood_name}.png"
...
def plot_roc(id_scores, ood_scores, ood_name):
    ...
    plt.title(f"ROC Curve: CIFAR-10 vs {ood_name.upper()} using Mahalanobis")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    save_path = f"results/roc_curves/roc_mahalanobis_cifar10_vs_{ood_name}.png"
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def main():
    ...
    id_scores = compute_mahalanobis_scores(
    ...
    for ood_name in ood_names:
        ...
        ood_scores = compute_mahalanobis_scores(
    ...
    save_path = "results/tables/mahalanobis_ood_results.csv"
    df.to_csv(save_path, index=False)

    print("=" * 70)
    print("Resultados Mahalanobis:")
